locals {
  encrypted_values = data.sops_file.secret_values.data

  jenkins_envirement_variables = [
    {
      env_name  = "SERVICE_NAME",
      env_value = "JENKINS"
    },
    {
      env_name  = "VPC_NAME",
      env_value = "vpc-bhunyd_apnortheast2"
    },
    {
      env_name  = "TARGET_GROUP_NAME",
      env_value = "jenkins-bhunydapne2-ext"
    },
    {
      env_name  = "SECURITY_GROUP_NAME",
      env_value = "jenkins-bhunyd_apnortheast2"
    },
    {
      env_name  = "EFS_NAME",
      env_value = "jenkins-efs-bhunyd_apnortheast2"
    },
    {
      env_name  = "ORG_NAME",
      env_value = "obhuny"
    },
    {
      env_name  = "TEAM_NAME",
      env_value = "admin"
    },
  ]
  demo_envirement_variables = [
    {
      env_name  = "SERVICE_NAME",
      env_value = "demoapp"
    },
    {
      env_name  = "VPC_NAME",
      env_value = "vpc-bhunyd_apnortheast2"
    },
    {
      env_name  = "TARGET_GROUP_NAME",
      env_value = "demoapp-bhunydapne2-ext"
    },
    {
      env_name  = "SECURITY_GROUP_NAME",
      env_value = "demoapp-bhunyd_apnortheast2"
    },
  ]
  test_envirnement_variables = []
}